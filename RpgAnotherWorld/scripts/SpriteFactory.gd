...
## Alle Monster haben eine Idle-Animation (AW-Rig oder DTII-Frames).
static func enemy_has_anim(id: String) -> bool:
	return aw_handles_enemy(id) or MON_DTII.has(id)

## Frame-Anzahl der Idle-Animation (AW-Rigs haben variable Zyklen).
static func enemy_anim_frames(id: String) -> int:
	if aw_handles_enemy(id):
		return aw_anim_frames(id, "idle")
	return ENEMY_FRAMES

static func enemy_frame(id: String, frame: int) -> Texture2D:
	# AW-Flächen-Rigs (Milestone 0: Schlotwerk) haben Vorrang.
	if aw_handles_enemy(id):
		return aw_enemy_tex(id, frame)
