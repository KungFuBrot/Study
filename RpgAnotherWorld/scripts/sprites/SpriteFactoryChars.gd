...
## anim: "idle" | "run" | "attack"/"cast" (AW-Rigs) | "aim" (Rax, Schusspose).
static func hero_battle_frame(id: String, frame: int, anim := "idle") -> Texture2D:
	if aw_handles_hero(id):
		return aw_hero_frame(id, anim, frame)
	if id == "rax":
...
## Frame-Anzahl einer Helden-Animation (AW-Rigs haben variable Zyklen).
static func hero_anim_frames(id: String, anim: String) -> int:
	if aw_handles_hero(id):
		return aw_anim_frames(id, anim)
	return 4

## Waffe eines Helden (kleines Overlay-Sprite), oder null.
## AW-Rigs tragen ihre Waffe IM Rig — kein Overlay nötig.
...
static func robot_battle_pose(pose: String, frame: int) -> Texture2D:
	if aw_handles_hero("rax"):
		return aw_hero_frame("rax", pose, frame)
