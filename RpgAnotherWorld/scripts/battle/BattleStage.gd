	...
	match arena_theme:
		...
		_:  # toxic
			return {
				...
				# AW-Kulisse: deutlich helleres Ambiente als die Höhlen-Arenen,
				# sonst saufen die flachen Farbflächen ab (Pilot-Lehre).
				"fg": Color(0.02, 0.04, 0.02), "ambient": Color(0.92, 0.96, 0.88),
			}
...
func _build_scene() -> void:
	...
	add_child(cm)
	# Schlotwerk-Kämpfe sind bereits auf den Another-World-Flächenstil
	# umgebaut; die übrigen Schauplätze behalten vorerst die Höhlen-Kulisse.
	if arena_theme == "toxic":
		_build_backdrop_aw()
	else:
		_build_backdrop_classic(pal)
	_finish_scene(pal)

## Another-World-Kulisse des Schlotwerks: harte Himmelsbänder, fahler Mond,
## Fabrik-Skyline mit driftenden Qualmfahnen, flacher Boden mit Giftlache.
## AW-Regel: Farben deutlich heller anlegen — Ambiente/Vignette/Grade dimmen.
func _build_backdrop_aw() -> void:
	# --- Himmel: vier harte Bänder, zum Horizont hin giftig glimmend.
	# Horizont bewusst HOCH (y=240): die Kampf-Formation staffelt in die Tiefe,
	# auch die hinterste Figur muss noch auf dem Boden stehen. ---
	var horizon := 240.0
	var bands: Array = [
		[0.0, 104.0, Color(0.13, 0.21, 0.33)],
		[104.0, 168.0, Color(0.19, 0.31, 0.43)],
		[168.0, 212.0, Color(0.26, 0.44, 0.52)],
		[212.0, horizon, Color(0.35, 0.58, 0.58)],
	]
	for b: Array in bands:
		var band := Polygon2D.new()
		band.polygon = PackedVector2Array([Vector2(-20, b[0]), Vector2(980, b[0]),
			Vector2(980, b[1]), Vector2(-20, b[1])])
		band.color = b[2]
		band.z_index = -20
		add_child(band)
	# --- Fahler Mond über der Heldenseite: harte Scheibe + Schattenfläche ---
	var moon := Polygon2D.new()
	moon.polygon = _poly_circle(Vector2.ZERO, 42.0, 26)
	moon.color = Color(0.82, 0.93, 0.88)
	moon.position = Vector2(508, 74)
	moon.z_index = -19
	add_child(moon)
	var moon_sh := Polygon2D.new()
	moon_sh.polygon = _poly_circle(Vector2.ZERO, 32.0, 22)
	moon_sh.color = Color(0.64, 0.78, 0.74)
	moon_sh.position = Vector2(495, 84)
	moon_sh.z_index = -19
	add_child(moon_sh)
	var halo := Sprite2D.new()
	halo.texture = SpriteFactory.circle(60, Color(0.55, 0.75, 0.68, 0.13))
	...
	halo.z_index = -19
	add_child(halo)
	# --- Ferne Fabrik-Skyline mit Schloten (flache Silhouette) ---
	var sky_far := Polygon2D.new()
	sky_far.polygon = PackedVector2Array([
		Vector2(-20, horizon), Vector2(-20, 158), Vector2(60, 158), Vector2(60, 114),
		Vector2(84, 114), Vector2(84, 158), Vector2(190, 164), Vector2(190, 78),
		Vector2(220, 78), Vector2(220, 164), Vector2(360, 168), Vector2(410, 146),
		Vector2(452, 146), Vector2(452, 168), Vector2(600, 172), Vector2(600, 108),
		Vector2(628, 108), Vector2(628, 172), Vector2(760, 174), Vector2(806, 154),
		Vector2(980, 160), Vector2(980, horizon)])
	sky_far.color = Color(0.15, 0.26, 0.33)
	sky_far.z_index = -18
	add_child(sky_far)
	# Qualmfahnen driften als flache Keile von den Schloten nach rechts
	for pl: Array in [[190.0, 78.0, 30.0], [600.0, 108.0, 28.0], [60.0, 114.0, 24.0]]:
		var plume := Polygon2D.new()
		plume.polygon = PackedVector2Array([
			Vector2(pl[0], pl[1]), Vector2(pl[0] + pl[2], pl[1]),
			Vector2(pl[0] + pl[2] + 120.0, pl[1] - 38.0), Vector2(pl[0] + pl[2] + 220.0, pl[1] - 48.0),
			Vector2(pl[0] + pl[2] + 110.0, pl[1] - 26.0), Vector2(pl[0] + pl[2] * 0.5, pl[1] - 6.0)])
		plume.color = Color(0.30, 0.44, 0.48, 0.85)
		plume.z_index = -18
		add_child(plume)
		var drift := plume.create_tween().set_loops()
		drift.tween_property(plume, "position:x", 16.0, 7.0 + pl[2] * 0.2) \
			.set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_IN_OUT)
		drift.tween_property(plume, "position:x", -16.0, 7.0 + pl[2] * 0.2) \
			.set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_IN_OUT)
	# --- Boden: flache Farbfläche mit heller Horizontkante, vorn dunkler ---
	var edge := Polygon2D.new()
	edge.polygon = PackedVector2Array([Vector2(-20, horizon), Vector2(980, horizon),
		Vector2(980, horizon + 7.0), Vector2(-20, horizon + 7.0)])
	edge.color = Color(0.46, 0.66, 0.54)
	edge.z_index = -13
	add_child(edge)
	var ground := Polygon2D.new()
	ground.polygon = PackedVector2Array([Vector2(-20, horizon + 7.0), Vector2(980, horizon + 7.0),
		Vector2(980, 560), Vector2(-20, 560)])
	ground.color = Color(0.22, 0.30, 0.24)
	ground.z_index = -13
	add_child(ground)
	var fore := Polygon2D.new()
	fore.polygon = PackedVector2Array([Vector2(-20, 470), Vector2(980, 484),
		Vector2(980, 560), Vector2(-20, 560)])
	fore.color = Color(0.13, 0.18, 0.14)
	fore.z_index = -12
	add_child(fore)
	# --- Giftlache unter der Gegnerseite mit pulsierenden Glühstreifen ---
	var pool := Polygon2D.new()
	pool.polygon = PackedVector2Array([Vector2(110, 396), Vector2(360, 388),
		Vector2(420, 412), Vector2(380, 436), Vector2(150, 440), Vector2(80, 418)])
	pool.color = Color(0.20, 0.38, 0.26)
	pool.z_index = -12
	add_child(pool)
	for st: Array in [[150.0, 414.0, 46.0], [230.0, 404.0, 70.0], [330.0, 416.0, 36.0]]:
		var streak := Polygon2D.new()
		streak.polygon = PackedVector2Array([
			Vector2(st[0], st[1]), Vector2(st[0] + st[2], st[1] - 2.0),
			Vector2(st[0] + st[2] - 4.0, st[1] + 3.0), Vector2(st[0] + 4.0, st[1] + 4.0)])
		streak.color = Color(0.55, 1.0, 0.40, 0.55)
		var smat := CanvasItemMaterial.new()
		smat.blend_mode = CanvasItemMaterial.BLEND_MODE_ADD
		streak.material = smat
		streak.z_index = -11
		add_child(streak)
		var puls := streak.create_tween().set_loops()
		puls.tween_property(streak, "modulate:a", 0.45, 1.4 + st[2] * 0.01) \
			.set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_IN_OUT)
		puls.tween_property(streak, "modulate:a", 1.0, 1.6 + st[2] * 0.01) \
			.set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_IN_OUT)
	# --- Flache Giftfass-Silhouetten am Rand ---
	for bar: Array in [[70.0, 452.0, 1.0], [872.0, 462.0, 1.15], [912.0, 452.0, 0.9]]:
		var barrel := Polygon2D.new()
		var bw := 26.0 * (bar[2] as float)
		var bh := 34.0 * (bar[2] as float)
		barrel.polygon = PackedVector2Array([
			Vector2(-bw * 0.5, 0), Vector2(bw * 0.5, 0), Vector2(bw * 0.55, -bh * 0.5),
			Vector2(bw * 0.5, -bh), Vector2(-bw * 0.5, -bh), Vector2(-bw * 0.55, -bh * 0.5)])
		barrel.color = Color(0.16, 0.24, 0.19)
		barrel.position = Vector2(bar[0], bar[1])
		barrel.z_index = -11
		add_child(barrel)
		var lid := Polygon2D.new()
		lid.polygon = PackedVector2Array([Vector2(-bw * 0.5, -bh), Vector2(bw * 0.5, -bh),
			Vector2(bw * 0.4, -bh - 4.0), Vector2(-bw * 0.4, -bh - 4.0)])
		lid.color = Color(0.42, 0.62, 0.36)
		lid.position = Vector2(bar[0], bar[1])
		lid.z_index = -11
		add_child(lid)

## Punkte eines n-Ecks (harter AW-„Kreis" ohne Anti-Aliasing-Weichheit).
func _poly_circle(c: Vector2, r: float, n: int) -> PackedVector2Array:
...
## Klassischer Höhlen-Hintergrund (gold/hate/void — noch nicht AW-umgebaut).
func _build_backdrop_classic(pal: Dictionary) -> void:
...
## Gemeinsamer Szenenabschluss: Fackeln, Lichter, Atmosphäre und die Kämpfer.
func _finish_scene(pal: Dictionary) -> void:
	# In der AW-Kulisse stehen die Fackeln auf dem Boden (Horizont y=240) —
	# auf Höhenlage 160 hingen sie sichtbar im Himmel.
	var torch_y := 286.0 if arena_theme == "toxic" else 160.0
	_add_torch(Vector2(70, torch_y), pal)
	_add_torch(Vector2(890, torch_y), pal)
	...
	for i in GameState.party.size():
		...
		s.set_meta("px_k", PXK_HERO_REF.get(data["id"], 4.8) / hscale)
	...
	for i in enemy_ids.size():
		...
		var esc := boss_sc if is_boss else 6.5
		# AW-Rigs sind fein aufgelöst → kleine Skala, px_k gleicht Aufsätze aus.
		if SpriteFactory.aw_handles_enemy(def["sprite"]):
			var aw_sc := SpriteFactory.aw_enemy_scale(def["sprite"])
			s.set_meta("px_k", esc / aw_sc)
			esc = aw_sc
		...
		# AW-Rigs sind auf ihre Zielhelligkeit gebacken — kaum zusätzlich dimmen.
		if SpriteFactory.aw_handles_enemy(def["sprite"]):
			dark = Color(1.0, 0.99, 1.0) if is_boss else Color(0.94, 0.92, 0.97)
...
## px_k gleicht die feiner aufgelösten AW-Rigs aus (siehe PXK_HERO_REF).
func _attach_shadow(s: Sprite2D, rx: int, ry: int, foot_y: float) -> void:
	var k: float = s.get_meta("px_k", 1.0)
	var sh := Sprite2D.new()
	sh.texture = SpriteFactory.shadow(int(round(rx * k)), maxi(int(round(ry * k)), 2))
...
## AW-Rigs backen auf großen Leinwänden — eigene Gesichts-Offsets (lokale px).
const AW_BOSS_FACE := {
	"boss": Vector2(38, -25),  # Auge des Schlotbarons (Schädel rechts)
}
...
func _attach_boss_life(s: Sprite2D, theme: String, sprite_id: String) -> void:
	...
	if SpriteFactory.aw_handles_enemy(sprite_id):
		face = AW_BOSS_FACE.get(sprite_id, Vector2.ZERO)
	...
	eyes.material = mat
	eyes.position = face
	eyes.scale = Vector2(eye_wide, 0.7) * k
	...
	breath.scale_amount_min = 0.010 * k
	breath.scale_amount_max = 0.022 * k
...
func _attach_menace(s: Sprite2D, home: Vector2, foot_local: float) -> CPUParticles2D:
	...
	# foot_local ist in Sprite-Pixeln — mit der echten Sprite-Skala in den
	# Weltraum umrechnen (AW-Rigs haben andere Skalen als DTII).
	mist.position = home + Vector2(0, foot_local * s.scale.y - 2.0)
	mist.amount = 7
	mist.lifetime = 2.6
	mist.preprocess = 2.6
